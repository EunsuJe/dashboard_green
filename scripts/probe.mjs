...
const dumpSchema = (label, schema) => {
  console.log(`\n=== ${label} ("${schema.title?.[0]?.plain_text ?? "?"}") 속성 정의 ===`);
...
// 1) ROOT_DS(담당자 테이블) 스키마 — 당사(Set)가 여기 직접 있는지 확인
const rootSchema = await g(`data_sources/${ROOT_DS}`);
dumpSchema("ROOT_DS(담당자)", rootSchema);

// 2) 조영재 행 찾기
const rows = (await q(ROOT_DS)).results;
const row = rows.find(r => Object.values(r.properties)
  .some(p => p.type === "title" && p.title.map(x => x.plain_text).join("").includes("조영재")));

if (!row) { console.log("조영재 행을 찾지 못함"); process.exit(0); }

console.log(`\n=== 조영재 행의 "당사" 관련 속성 원본 값 ===`);
...
console.log(`\n조영재 → ${relKey} ${ids.length}건`);

// 3) 중간 연결 테이블(프로젝트 참여 기여율) 스키마 — 당사 관련 속성이 있는지 확인
const targetDs = rootSchema.properties[relKey]?.relation?.data_source_id;
if (targetDs) {
  const target = await g(`data_sources/${targetDs}`);
  dumpSchema(`중간 연결 DS("${relKey}")`, target);
}

// 4) 자식(중간 연결) 페이지들의 실/진/당사 관련 원본 값
console.log(`\n=== 자식 ${ids.length}건의 실/진/당사 값 ===`);
for (const id of ids) {
